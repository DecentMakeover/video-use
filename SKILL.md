...
- **`render.py <edl.json> -o <out>`** — per-segment extract → concat → overlays (PTS-shifted) → subtitles LAST. `--preview` for a 1080p evaluable preview; `--draft` for a 720p cut-point check. Hardware H.264 is opt-in with `--encoder videotoolbox` (`--vt-bitrate M` overrides its bitrate); `--hwdecode` separately opts into VideoToolbox decoding, and `--long-edge PIXELS` overrides resolution. `--build-subtitles` generates master.srt inline; add `--subtitle-case natural` to preserve Scribe capitalization.
